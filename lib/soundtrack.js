...
var Soundtrack = function(app) {
  ...
  this.youtube = new YouTube(app.config.youtube.token);
  this.slack = new SlackAPI(app.config.slack.token, app.config.slack.botUsername, app.config.slack.botEmoji);
};
...
Soundtrack.prototype.sortPlaylist = function() {
  ...
  if (!app.config.settings.sortPlaylists) return;
...
Soundtrack.prototype.gatherSources = function(track , callback) {
  ...
  Artist.populate( track , {
    ...
    async.parallel([
      ...
      function(done) {
        rest.get('https://api.soundcloud.com/tracks.json?limit='+maxTracks+'&client_id='+app.config.soundcloud.id+'&q='+query).on('complete', function(data) {
          var functions = [];
